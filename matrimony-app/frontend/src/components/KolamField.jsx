 ...
 * brief asks for via parallax + soft glow.
 */
export default function KolamField({ className = '' }) {
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      points = Array.from({ length: Math.max(24, Math.min(count, 90)) }, () => ({
        x: Math.random() * width,
    ...
    function step() {
      ...
      for (const p of points) {
        ...
        grad.addColorStop(0, 'rgba(128,0,0,0.65)');
